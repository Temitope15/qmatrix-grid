...
<div align="center">
minimize &nbsp;&nbsp;&nbsp; &Sigma;<sub><i>i</i></sub> <i>C<sub>i</sub></i>(<i>P<sub>i</sub></i>)<br>
subject to &nbsp;&nbsp; &Sigma;<sub><i>i</i></sub> <i>P<sub>i</sub></i> = <i>D</i> &nbsp;&nbsp;&nbsp; (power balance)<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <i>P</i><sub>min,<i>i</i></sub> &le; <i>P<sub>i</sub></i> &le; <i>P</i><sub>max,<i>i</i></sub> &nbsp;&nbsp; (generator limits)
</div>

With the cubic cost <i>C<sub>i</sub></i>(<i>P</i>) = <i>aP</i><sup>3</sup> + <i>bP</i><sup>2</sup> + <i>cP</i> + <i>d</i>, the objective is a degree-three polynomial and the power-balance constraint &Sigma;<sub><i>i</i></sub> <i>P<sub>i</sub></i> = <i>D</i> is expressed directly by the device `sum_constraint`. We apply the substitution <i>P<sub>i</sub></i> = <i>P</i><sub>min,<i>i</i></sub> + <i>y<sub>i</sub></i> with <i>y<sub>i</sub></i> &ge; 0, so lower generation limits are satisfied by construction and the sum constraint becomes &Sigma;<sub><i>i</i></sub> <i>y<sub>i</sub></i> = <i>D</i> &minus; &Sigma;<sub><i>i</i></sub> <i>P</i><sub>min,<i>i</i></sub>. The problem is executed on Dirac-3 hardware through the `eqc-models` `PolynomialModel` and `Dirac3ContinuousCloudSolver`. All classical baselines use the same grid model and the SciPy HiGHS and SLSQP solvers.
...
**Model B — non-convex valve-point cubic.** Thermal units with multiple steam-admission valves exhibit valve-point loading, producing a non-monotonic marginal cost. We represent this with a cubic whose marginal cost <i>m</i>(<i>P</i>) = 3<i>aP</i><sup>2</sup> + 2<i>bP</i> + <i>c</i> attains its minimum strictly inside each unit's operating range [<i>P</i><sub>min</sub>, <i>P</i><sub>max</sub>]. Coefficient magnitudes follow the standard valve-point dispatch benchmarks (Walters and Sheble, *IEEE Trans. Power Systems* 1993; Sinha, Chakrabarti and Chattopadhyay, *IEEE Trans. Evolutionary Computation* 2003). Only the cost shape is calibrated from the literature; the generator count and capacities are taken from the dataset. Non-convexity is confirmed per unit rather than assumed: the second derivative of each cost curve is negative over part of the operating range.
...
**Encoding.** Ten continuous modes (one per generator), 30 polynomial terms up to degree three, full connectivity, `num_samples = 20`, `relaxation_schedule = 3`, and the device `sum_constraint` set to <i>D</i> &minus; &Sigma;<sub><i>i</i></sub> <i>P</i><sub>min,<i>i</i></sub>. The Dirac allocation is unmetered, so all scenarios and the control were run on hardware.
